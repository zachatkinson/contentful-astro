...
export const richTextOptions:Options = {
    ...
    renderNode: {
        [BLOCKS.PARAGRAPH]: (node, children) => {
            return `<p class="mb-4 text-sm leading-6 text-black">${children}</p>`;
        },
        [BLOCKS.HEADING_1]: (node, children) => {
            return `<h1 class="text-2xl font-bold my-4">${children}</h1>`;
        },
        [BLOCKS.HEADING_2]: (node, children) => {
            return `<h2 class="text-xl font-semibold my-3">${children}</h2>`;
        },
        [BLOCKS.UL_LIST]: (node, children) => {
            return `<ul class="list-disc list-inside my-2">${children}</ul>`;
        },
        [BLOCKS.OL_LIST]: (node, children) => {
            return `<ol class="list-decimal list-inside my-2">${children}</ol>`;
        },
        ...
        [INLINES.HYPERLINK]: (node, children) => {
            ...
            return `<a href="${url}" class="underline text-blue-600 hover:text-blue-800">${children}</a>`;
        },
    },
};